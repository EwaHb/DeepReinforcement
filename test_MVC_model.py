...
T1 = time.time()
action = None
[idx1, idx2] = mvc.get_ilegal_actions(state, action)
while done == False:
    G = state.g
    [pi, val] = NN(G)
    pi = pi.squeeze()
    ...
    oldAction = action
    action = dist.sample()
    new_state, reward, done = mvc.step(state, action, oldAction)
    [idx1, idx2] = mvc.get_ilegal_actions(new_state, action)
    state = new_state
    sum_r += reward
T2 = time.time()
